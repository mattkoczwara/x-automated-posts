...
def fetch_btc_data():
    """
    Fetches the past 30 days of Bitcoin-USD price data from CoinGecko,
    ...
    """
    print("[INFO] Fetching BTC data (30 days) from CoinGecko...")
    url = "https://api.coingecko.com/api/v3/coins/bitcoin/market_chart"
    params = {
        "vs_currency": "usd",
        "days": 30  # last 30 days for monthly data
    }
    ...
    print(f"[INFO] Successfully retrieved {len(btc_data)} data points for BTC (30 days).")
    return btc_data
...
def generate_chart_image(btc_data, filename="chart.png"):
    ...
    # We'll keep "%m-%d %H:%M" so you can see date+hour
    labels = [dt.strftime("%m-%d %H:%M") for dt, _ in btc_data]
...
def main():
    try:
        # A) Fetch BTC data (30 days) in ET
        btc_data = fetch_btc_data()

        # B) (Optional) Downsample if needed to avoid huge payloads
        btc_data = btc_data[::5]
        ...
        # D) Generate the dark-mode monthly chart
        ...
        print("[INFO] Posting tweet with monthly BTC price chart...")
