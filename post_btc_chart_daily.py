...
def main():
    try:
        # Step A: Fetch BTC data (24h) in ET
        btc_data = fetch_btc_data()

        # Step B: (Optional) Downsample if needed to avoid large payloads, e.g. every 5th data point
        btc_data = btc_data[::5]
        print(f"[INFO] Downsampled data to {len(btc_data)} points.")

        # Step C: Compute price stats
        ...
        # Step D: Generate the dark-mode chart
        chart_filename = "chart.png"
        generate_chart_image(btc_data, filename=chart_filename)

        # Step E: Determine appropriate emoji(s) for the price movement
        ...
        # Step F: Post tweet with chart
        print("[INFO] Posting tweet with BTC price data and dark mode chart...")
